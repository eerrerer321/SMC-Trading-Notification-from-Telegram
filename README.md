...
### 回測績效參考（ETHUSDT 2020-2025）

| 年份 | 類型 | 報酬率 | 交易數 | 勝率 | MDD | PF |
|------|------|--------|--------|------|-----|-----|
| 2020 | 樣本外 | **+584.36%** | 85 | 55.3% | 27.6% | 2.66 |
| 2021 | 樣本外 | **+795.72%** | 89 | 57.3% | 54.3% | 2.32 |
| 2022 | 樣本外 | **+277.73%** | 103 | 58.3% | 17.3% | 2.06 |
| 2023 | 樣本內 | **+37.68%** | 79 | 50.6% | 31.1% | 1.47 |
| 2024 | 樣本內 | **+34.95%** | 90 | 48.9% | 15.1% | 1.33 |
| 2025 | 樣本內 | **+125.44%** | 94 | 52.1% | 32.3% | 1.66 |

**總計**: 樣本外 +1657.81% / 樣本內 +198.07% / 全部 **+1855.89%**

> **注意**: 歷史績效不代表未來表現，僅供參考。樣本外數據 (2020-2022) 未參與參數優化過程。
...
STRATEGY_PARAMS = {
    ...
    'ob_quality_threshold': 45,      # [優化] OB 質量門檻 50→45
    'min_pullback_pct': 0.25,        # [優化] 回撤要求 0.3→0.25
    'risk_reward_ratio': 3.5,        # [優化] 風險報酬比 2.5→3.5
...
**A**: 勝率不是唯一重要的指標。配合 3.5 倍的風險報酬比，即使勝率 50%，期望值仍為正：
```
0.50 × 3.5 - 0.50 × 1 = +1.25（每筆平均賺 1.25 倍風險）
```
